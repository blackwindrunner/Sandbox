...
if ($MY_COMMITDATE ne "") {
  ...
  if (-e $commit_path_base){
    ...
    if (-f "$commit_path\\$zip_name") {
      ...
      my $MY_LAST_ZIPDATE = my_convert_timestamp ($zip_name);
      #print "DEBUG: MY_LAST_COMMITDATE: $MY_LAST_ZIPDATE";
      ...
      my $cmd = "report -g levelview -family $family -wh \"releasename='$release' and state in ('build','commit','complete') and commitdate>'$MY_LAST_ZIPDATE' order by commitdate\" -sel \"name\"";
      ...
      if (! @levels) {
        ...
        my $MY_BUILD_LEVEL;
        foreach $MY_BUILD_LEVEL (@levels) { 
          chomp $MY_BUILD_LEVEL;
          print "*************************lwhtest*******************";
          print "DEBUG: MY_BUILD_LEVEL: $MY_BUILD_LEVEL \n";
          ExtractPartsInLevel($family, $release, $MY_BUILD_LEVEL, $component); 
        }
      }
    ...
    if ($CACHE_OUT_OF_DATE == 1) {
      ...
      system("test -d $commit_path || mkdir $commit_path");
      print "\t\t cd $file_path && zip -r $commit_path\\$MY_LEVELDATE * \n\n";
      system ("cd $file_path && zip -r $commit_path\\$MY_LEVELDATE * ");
    ...
    #if ($MY_COMMITDATE eq $MY_COMMITDATE_CHECK){                 lwh
      #print "\t\t commit date is matching \n\n";
    #}else{
