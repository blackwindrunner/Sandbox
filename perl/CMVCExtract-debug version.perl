...
# 07/28/2004 - lisa2 - speed up CMVC operation
#-------------------------------------------------------------------------------------------------------
...
print "0 echo file_path=$file_path release=$release  level=$level component=$component fileName=$fileName family=$family \n\n";

#the folder which contains local cmvc extract archive/cache
my $commit_path_base = "D:\\composer811_base";
print "1 echo $commit_path_base \n\n";
my $commit_path = "$commit_path_base\\$component";
print "2 echo $commit_path \n\n";
...
print "3 echo ---$MY_COMMITDATE--- \n\n \t\t =========&my_determine_commit_date($family, $release)======\n\n";
print "4 echo commit_path_base=$commit_path_base \n\n";
...
#  if ($MY_LEVELDATE = $MY_COMMITDATE){ 
#  print "6 echo $MY_LEVELDATE \n\n";}
#  print "\n\n 6 echo commit_path_base=$commit_path_base ";
  ...
  if (-e $commit_path_base){
    ...
    if ($CACHE_OUT_OF_DATE == 1) {
      ...
      system("test -d $commit_path || mkdir $commit_path");
      print "\t\t test -d $commit_path || mkdir $commit_path  succeed \n\n";
...
#my $cmd = "Report -g FileView -family $family -where \"releaseName = 'composer611' and compName = '$component' and dropDate is null and VersionSID is not null order by pathName\" -select \"VersionSID,pathName,nuPathName\"";
print "\t$cmd\n"; #  if ($verbose or $showOnly);
open(QUERY, "$cmd |");
@fileNames = <QUERY>;         
close(QUERY);
